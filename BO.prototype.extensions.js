...
Object.extend(Event, {
	...
	isOpenInNewTabClick: function (event) {
		return (event.ctrlKey || event.metaKey || Event.isMiddleClick(event));
	}
});
// Element extensions
Element.addMethods({
	...
	, setBorderRadius: function (element) {
		...
		element.setStyle({ 'border-radius': rads, '-webkit-border-radius': rads, '-moz-border-radius': rads });
		return element;
	}
	...
	, getParameter: function (element, paramName, defaultValue) {
		var rt = element.retrieve(paramName, defaultValue) || element.readAttribute(paramName);
		element.store(paramName, rt);
		return rt;
	}
	// groups children of element in columns given the criteria
	// criteria can be a number of column, or a pixel width (number of columns will be deducted)
	...
	, centerTopViewport: function (element) {
