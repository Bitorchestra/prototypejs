...
var Prototype = {
  ...
  Browser: (function(){
    ...
    return {
      ...
      Mobile:         /iphone|ipad|ipod|android|blackberry|mini|windows\sce|iemobile|palm/i.test(ua),
      MobileAndroid:        /android/i.test(ua),
      MobileBlackBerry:     /blackberry/i.test(ua),
      MobileiOS:            /iphone|ipad|ipod/i.test(ua),
      MobileOpera:          /opera mini/i.test(ua),
      MobileWindows:        /iemobile|windows\sce/i.test(ua)
    }
  })(),
...
Ajax.Request = Class.create(Ajax.Base, {
  ...
  request: function(url) {
    ...
    if (params && (this.method === 'get' || (this.method === 'post' && this.options.postBody))) {
...
var operators = {
  ...
  '$=': function(attr, val) {
    var i = attr.indexOf(val)
    return i >= 0 && attr.indexOf(val) + val.length === attr.length;
  },
  '!=': function(attr, val) {
    return attr !== val;
  }
...
(function(GLOBAL) {
  ...
  function hasObservers(element, eventName) {
    element = $(element);
    var registry = getRegistryForElement(element);
    var evts = (registry ? registry[eventName] : null) || [];
    if (eventName == 'click' && (
        (element.tagName == 'A' && element.hasAttribute('href'))
        || ['INPUT','SELECT','TEXTAREA','BUTTON'].member(element.tagName)
    )) evts.add[null];
    return (evts.size() > 0 || element.hasAttribute('on' + eventName));
  }
  ...
  function fireEvent_IE(element, eventName, memo, bubble) {
    ...
    try {
        ...
        // Error: No such interface supported (damn IE)
  ...
  Object.extend(document, {
    ...
    hasObservers:   hasObservers.methodize(),
